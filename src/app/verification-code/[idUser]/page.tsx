...
export default function PageVerificationCode({ params }: { params: { idUser: string } }) {
    ...
    async function verifyIdentity(code: string) {
        // se realiza la solicitud de verificación de identidad con el código de verificación
        try {
            console.log(params.idUser)
            await authService.verifyIdentity(params.idUser, code)
            // si no ocurre ningún error entonces se redirige a la página del cambio de contraseña
            router.push('/change-password/' + params.idUser)
        } catch (error) {
            if (error instanceof Error)
                alert(error.message)
            console.log(error)
